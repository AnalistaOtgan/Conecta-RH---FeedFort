...
import { PlusCircleIcon, ChevronDownIcon, EditIcon, TrashIcon } from '../components/icons';
...
const Employees: React.FC<EmployeesProps> = () => {
  ...
  const [newEmployee, setNewEmployee] = useState({ name: '', email: '', role: '', sector: '', shift: '' });
  ...
  const handleAddEmployee = () => {
      ...
      setNewEmployee({ name: '', email: '', role: '', sector: '', shift: '' });
  };

  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-brand-text">Funcionários</h2>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Novo Funcionário">
        <div className="space-y-4">
            ...
            <input type="email" placeholder="Email" value={newEmployee.email} onChange={(e) => setNewEmployee({...newEmployee, email: e.target.value})} className="w-full p-2 border rounded-md"/>
